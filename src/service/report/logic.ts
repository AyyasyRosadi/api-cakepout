...
class Logic extends LogicBase {
  ...
  private async getAccountByGroup(
    ...
    for (let i in accounts) {
      ...
      if (monthIndex < 6) {
        for (let j = 6; j < 12; j++) {
          const oneLedger = await ledger.getOneLedgerByOpenClosed(
            year,
            j,
            accounts[i]?.uuid!
          );
  ...
  public async getBalanceSheetReport(
    ...
    const labaRugi =
      (await this.getGroupBalanceReport(4, monthIndex)).finalAmount -
      (await this.getGroupBalanceReport(5, monthIndex)).finalAmount;
