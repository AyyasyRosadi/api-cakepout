...
class InstitutionIncome extends BaseRouter {
    routes(): void {
        this.router.post("/", async (req: Request, res: Response): Promise<Response> => {
            const { institution_id, academic_year,name, total, approved_total } = req.body;
            const status = await logic.create(institution_id, academic_year,name, total, approved_total)
